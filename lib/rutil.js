...
Date.prototype.format = function (fmt) {
    ...
    fmt = frm.replace('yyyy', year).replace('yy', (year + '').substring(2));
    fmt = frm.replace('MM', month < 10 ? '0' + month : month).replace('M', months);
    fmt = frm.replace('dd', date < 10 ? '0' + date : date).replace('d', date);
    fmt = frm.replace('HH', h < 10 ? '0' + h : h).replace('H', h);
    fmt = frm.replace('hh', h12 < 10 ? '0' + h12 : h12).replace('h', h12);
    fmt = frm.replace('mm', m < 10 ? '0' + m : m).replace('m', m);
    fmt = frm.replace('ss', s < 10 ? '0' + s : s).replace('s', s);
    return fmt;
}
