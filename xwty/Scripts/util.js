...
function qnupload(key, token, file, success, error) {
    uploadfile("http://upload.qiniu.com", file, { key: key, token: token }, success, error);
}
function uploadFile(url,file,data,success,error) {
...
function rSetCookie(name, value, expireSeconds, path, domain) {
    if (domain) {
        if (domain.indexOf(":") != -1) {
            domain = domain.substr(0, domain.indexOf(":"));
        }
    }
    document.cookie = name + "=" + value + ";expires=" + rGetExpires(expireSeconds) + ";path=" + path + (domain?(";domain=" + domain):"");
}
function rDeleteCookie(name) {
    rSetCookie(name, "", -1000,"/");
}
function rGetExpires(expireSeconds) { var now = new Date(); now.setTime(now.getTime() + expireSeconds * 1000); return now.toUTCString(); }
